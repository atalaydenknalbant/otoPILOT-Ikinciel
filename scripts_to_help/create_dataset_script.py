...
"""
    ...
    # KLASÖRDEKİ TÜM JSON DOSYALARINDAN MANUEL ÖRNEKLERİ TOPLA
    input_folder = r"C:\Users\yineh\OneDrive\Masaüstü\ilk"
    json_paths = sorted(glob.glob(os.path.join(input_folder, "*.json")))

    if not json_paths:
        print(f"Uyarı: Klasörde json bulunamadı. Klasör: {input_folder}")
    ...
    for p in json_paths:
        ...
        if isinstance(data, list):
            for ex in data:
                # ex beklenen format: {"user": "...", "assistant": {...}}
                dataset.append({
                    "system": system_message,
                    "user": ex.get("user", ""),
                    "assistant": json.dumps(ex.get("assistant", {}), ensure_ascii=False)
                })
            total_in += len(data)
        else:
            # Tek obje gelirse yine ekleyelim
            dataset.append({
                "system": system_message,
                "user": data.get("user", ""),
                "assistant": json.dumps(data.get("assistant", {}), ensure_ascii=False)
            })
            total_in += 1

    # SENTETİK ÜRETİM BLOĞU KAPATILDI
    """
    # --- BURADAKİ TÜM SENTETİK ÖRNEK ÜRETİMİ ARTIK DEVRE DIŞI ---
    # fiyatlar_satis = [...]
    # fiyatlar_kiralik = [...]
    # yillar = [...]
    # ... tamamı kaldırıldı ...
    ...
    output_filename = r"C:\Users\yineh\OneDrive\Masaüstü\instruct_arabam_dataset_manual_only.parquet"
    df.to_parquet(output_filename, index=False)

    print(f"Toplam giriş örneği: {total_in}")
    print(f"Oluşturulan satır: {len(df)}")
...
#     with open(r"C:\Users\yineh\OneDrive\Masaüstü\ilk\birlesmis_duzenlenmis_7.json", "r", encoding="utf-8") as f:
...
#                 province_list = list(LOCATIONS.keys())
#                 province = random.choice(province_list)

#                 if search_type == "province_only":
#                     allow_multi = random.random() < 0.5 and len(province_list) > 1
#                     if allow_multi:
#                         max_cities = min(3, len(province_list))
#                         n_cities = random.randint(2, max_cities)
#                         provinces = random.sample(province_list, k=n_cities)
#                         assistant_json["il"] = provinces
#                         query_parts.extend(provinces)
#                     else:
#                         assistant_json["il"] = [province]
#                         query_parts.append(province)

#                 elif search_type == "district_only":
#                     districts = [d for d in LOCATIONS.get(province, []) if d != "Merkez"]
#                     if districts:
...
#     df = pd.DataFrame(dataset)
#     output_filename = r"C:\Users\yineh\OneDrive\Masaüstü\instruct_arabam_dataset_130k_v2.parquet"
#     df.to_parquet(output_filename, index=False)
